...
class ChartDataset(torch.utils.data.Dataset):
    def __init__(self, root, num_samples=None, transforms=None):
        ...
        # Filter samples to ensure equal distribution among chart types
        self.imgs, self.annots, self.type_counts = self.filter_samples(self.imgs, self.annots, num_samples)
        ...
        for img, annot in zip(imgs, annots):
            ...
            with open(annot_path) as f:
                ...
                if chart_type in chart_dict:
                    chart_dict[chart_type].append((img, annot))
    ...
    def __getitem__(self, idx):
        ...
        for item in annot['text']:
            if item['role'] == 'tick_label':
                ...
                if x_max - x_min > y_max - y_min:  # Horizontal tick (x_tick_labels)
                    labels.append(2)  # Label for x_tick_labels
                else:  # Vertical tick (y_tick_labels)
                    labels.append(1)  # Label for y_tick_labels
...
if __name__ == '__main__':
    ...
    dataset = ChartDataset(root='train', num_samples=num_samples, transforms=transform)


    # Split the dataset into train, validation, and test sets, ensuring balance
    def balanced_split(dataset, num_samples, val_ratio=0.1, test_ratio=0.1):
        indices_by_type = defaultdict(list)
        for idx, (img, annot) in enumerate(zip(dataset.imgs, dataset.annots)):
            annot_path = os.path.join(dataset.root, "annotations", annot)
            with open(annot_path) as f:
                data = json.load(f)
                chart_type = data['chart-type']
    ...
    # Function to count samples of each type in a dataset
    def count_types(dataset, dataset_indices):
        type_counts = {chart_type: 0 for chart_type in chart_types}
        for idx in dataset_indices:
            annot_path = os.path.join(dataset.root, "annotations", dataset.annots[idx])
            with open(annot_path) as f:
                data = json.load(f)
                chart_type = data['chart-type']
    ...
    # Print counts for train, val, and test sets
    print("Training set type counts:", count_types(dataset, train_indices))
    print("Validation set type counts:", count_types(dataset, val_indices))
    print("Test set type counts:", count_types(dataset, test_indices))
    ...
    # Model and optimizer
    model = get_model(num_classes=3)  # 3 classes: background, y_tick_labels, x_tick_labels
    ...
    class EarlyStopping:
        ...
        def save_checkpoint(self, val_loss, model):
            ...
            if self.verbose:
                ...
                pass
            torch.save(model.state_dict(), 'axis_bounding_box_detector_500.pth')
