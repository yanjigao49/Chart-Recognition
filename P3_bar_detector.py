...
chart_types = ['vertical_bar', 'horizontal_bar']

class ChartDataset(torch.utils.data.Dataset):
    def __init__(self, root, num_samples=50, transforms=None):
        self.root = root
        self.transforms = transforms
        self.imgs, self.annots = self.load_samples(num_samples)

    def load_samples(self, num_samples):
        imgs = list(sorted(os.listdir(os.path.join(self.root, "images"))))
        annots = list(sorted(os.listdir(os.path.join(self.root, "annotations"))))
        ...
        for img, annot in zip(imgs, annots):
            ...
            with open(annot_path) as f:
                ...
                if chart_type in chart_dict and len(chart_dict[chart_type]) < samples_per_type:
                    chart_dict[chart_type].append((img, annot))
                if all(len(chart_dict[chart_type]) >= samples_per_type for chart_type in chart_types):
                    break

        final_imgs, final_annots = [], []
        for chart_type in chart_types:
            samples = chart_dict[chart_type]
            final_imgs.extend([img for img, _ in samples])
            final_annots.extend([annot for _, annot in samples])

        return final_imgs, final_annots
    ...
    def __getitem__(self, idx):
        ...
        for bar in annot['visual-elements']['bars']:
            x0, y0, width, height = bar['x0'], bar['y0'], bar['width'], bar['height']
            boxes.append([x0, y0, x0 + width, y0 + height])
            labels.append(1)  # Label for bars
...
if __name__ == '__main__':
    num_samples = 50  # Define the number of samples to use
    ...
    # Model and optimizer
    model = get_model(num_classes=2)  # 2 classes: background, bars
    ...
    class EarlyStopping:
        ...
        def save_checkpoint(self, val_loss, model):
            ...
            if self.verbose:
                pass
            torch.save(model.state_dict(), 'bar_detector_500.pth')
