...
class LineDataset(torch.utils.data.Dataset):
    def __init__(self, root, num_samples=None, transforms=None):
        ...
        # Filter samples to ensure only line charts
    ...
    def filter_samples(self, imgs, annots, num_samples):
        ...
        print(f"line samples: {len(line_imgs)}")
        return line_imgs, line_annots
    ...
    def __getitem__(self, idx):
        ...
        boxes = []
        labels = []
        for line in annot['visual-elements']['lines']:
            for point in line:
                x = point['x']
                y = point['y']
                box_size = 18  # Define the size of the box around the line
                x_min = x - box_size / 2
                y_min = y - box_size / 2
                x_max = x + box_size / 2
                y_max = y + box_size / 2
                boxes.append([x_min, y_min, x_max, y_max])
                labels.append(1)  # Label for lines
...
if __name__ == '__main__':
    ...
    # Load the line dataset
    dataset = LineDataset(root='train', num_samples=num_samples, transforms=transform)
    ...
    # Model and optimizer
    model = get_model(num_classes=2)  # 2 classes: background, line
    ...
    class EarlyStopping:
        ...
        def save_checkpoint(self, val_loss, model):
            ...
            if self.verbose:
                ...
                pass
            torch.save(model.state_dict(), 'line_detector.pth')
