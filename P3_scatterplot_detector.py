...
class ScatterDataset(torch.utils.data.Dataset):
    def __init__(self, root, num_samples=None, transforms=None):
        ...
        # Filter samples to ensure only scatter charts
        ...
        for img, annot in zip(imgs, annots):
            ...
            with open(annot_path) as f:
                ...
                if data['chart-type'] == 'scatter':
                    scatter_imgs.append(img)
                    scatter_annots.append(annot)
                    count += 1
                    if count >= num_samples:
                        break

        print(f"scatter samples: {len(scatter_imgs)}")
        return scatter_imgs, scatter_annots
    ...
    def __getitem__(self, idx):
        ...
        boxes = []
        labels = []
        for scatter in annot['visual-elements']['scatter points']:
            for point in scatter:
                x = point['x']
                y = point['y']
                box_size = 10  # Define the size of the box around the scatter
                x_min = x - box_size / 2
                y_min = y - box_size / 2
                x_max = x + box_size / 2
                y_max = y + box_size / 2
                boxes.append([x_min, y_min, x_max, y_max])
                labels.append(1)  # Label for scatters
...
if __name__ == '__main__':
    ...
    # Load the scatter dataset
    dataset = ScatterDataset(root='train', num_samples=num_samples, transforms=transform)
    ...
    # Model and optimizer
    model = get_model(num_classes=2)  # 2 classes: background, scatter
    ...
    class EarlyStopping:
        ...
        def save_checkpoint(self, val_loss, model):
            ...
            if self.verbose:
                ...
                pass
            torch.save(model.state_dict(), 'scatter_detector_500.pth')
